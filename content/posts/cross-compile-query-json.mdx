...
query-json uses `esy` as a package manager that helps to manage either native OCaml and Reason from opam registry or JavaScript packages from npm registry; and `dune` as a build system.

All of the build process and their tests runs on our CI in Github Actions, which allows running Mac, Windows and Linux images to compile with the OCaml compiler to different architectures.

Once the build succeeds it pushes the binary into Github Releases and npm registry.

## How to compile to the Web
The compilation to JavaScript is the sweet section of this blog post since we are using OCaml (under the hood while writing Reason), it allows us to compile directly to JavaScript using [js_of_ocaml](https://github.com/ocsigen/js_of_ocaml).

Using js_of_ocaml in dune is as easy as adding a flag in the dune file. Using the same build command you have an artefact in JavaScript 🤯 . It will compile any library to JavaScript.
...
jsoo wasn't created with the same mentality as most Frontend developers solve their problems today and was born 10 years ago to run some OCaml into a browser.
...
This isn't particular to jsoo, is more related to use jsoo and BuckleScript together. In order to run the jsoo artefact into the BuckleScript/Reason codebase, I needed to write bindings for it.
...
If you are writing an OCaml backend and your team is familiar with it. Your project would benefit from sharing code, don't require a hard need on a lower bundlesize and want to carry all OCaml dependencies, ppx and patterns, jsoo it's the best option.
...
<!-- TODO: ? -->
<!-- TODO: Add pictures:
https://unsplash.com/photos/4M09z_Mqw_Q
https://unsplash.com/photos/2VPYhLUQsdQ
https://unsplash.com/photos/NbXjKw01ZuA
https://unsplash.com/photos/io7dX_1EFCg
https://unsplash.com/photos/8RMP683VW-o
https://unsplash.com/photos/6HZXZOf9_4E
https://unsplash.com/photos/cmptaciE_-M
https://unsplash.com/photos/Vj1OqmW4oh8
 -->