...
I work at Ahrefs, as a Software engineer on their tooling. Maintaining their design system, working on styled-ppx, helping with [Melange](https://github.com/melange-re/melange) and now working on server-reason-react.

Ahrefs is a comprehensive SEO toolset that provides data-driven insights and competitive analysis for digital marketers and businesses. With one of the best crawlers of the internet, and we recently launched a search engine called [Yep.com](https://yep.com/about).
...
The actual string generation is a simple walking down the react component tree and generate specific HTML representation of each node, taking into account a few details such as serialize DOM attributes, process inline styles, encode HTML and a few React particularities (such as `dangerouslySetInnerHTML` or hydration hacks).
...
It shows an approximate improvement of x7 over Node and x3.5 over Bun.
...
The same code gets compiled to Native and JavaScript (thanks entirely to [Melange](https://github.com/melange-re/melange)). As far as I know, it can’t be done in other native languages such as Rust or Go.
