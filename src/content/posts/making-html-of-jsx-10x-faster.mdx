...
```
Fully static element: <div class="container"></div>

Baseline (JSX.node):     ~8M renders/sec
After:                  ~27M renders/sec   → ~3x faster
```

For deeply nested static elements, the improvement is more dramatic:

```
Nested static: <div><header><h1>Title</h1></header><main>...</main></div>
...
### Zero-allocation escaping

The naive implementation of `JSX.escape` allocates a buffer and copies every character, which is slow.

Most user-generated content doesn't contain HTML special characters (`<`, `>`, `&`, `"`, `'`).

I implemented a "scan-first" strategy:
1. Scan the string.
2. If no special characters are found, return the original string **(0 allocations)**.
3. Only allocate a buffer if escaping is actually needed.

Allocations are expensive; reading memory is cheap. Scanning the string twice in the rare case is a small price to pay for zero allocations in the common case.
