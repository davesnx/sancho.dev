...
description: "What Reason gives us beyond “it's just JSX”"
...
mlx wins in maintenance. It's OCaml's parser with [the JSX rules](https://github.com/ocaml-mlx/mlx/blob/main/mlx/parser.mly), and [ocamlformat-mlx](https://github.com/ocaml-mlx/ocamlformat-mlx) is ocamlformat with a small patch. For Reason, each OCaml release means adapting Reason to the changes. mlx has less separate syntax to maintain.

I also want the story to be clearer. Melange, ReScript, Reason, BuckleScript, js_of_ocaml: people who are not familiar with these projects struggle to understand the differences, and I don't blame them. The ecosystem is modular, which may be great for experts, but alien for newcomers. With mlx, you just write OCaml (with a bit of JSX).

This fits the wider work on Dune and Melange, and my work on Universal JS and Universal Belt. I want libraries to be easy to compile to both JavaScript and native executables. Today we rely on `copy_files` and custom directories for that. The single-context work in Dune is part of making this simpler. Reason benefits from that work too; mlx gives us a clearer way to describe the language we are using.

Reason has been in maintenance mode for a few years, and the momentum is long gone. It's sad to say that about a project I believed in and helped maintain. But maybe mlx is the next chapter we should work on. What do you think?
