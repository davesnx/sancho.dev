...
- [ ] Look for “allow” “improvement” and other repeated sentences that sound the same. Check the style of presenting a bunch of features from [https://kaleidawave.github.io/posts/introducing-ezno](https://kaleidawave.github.io/posts/introducing-ezno/)
- [ ] Explain the awesomeness from the voice https://vercel.com/blog/turborepo-migration-go-rust or https://turbo.build/pack/docs/why-turbopack#bundling-vs-native-esm
...
This blog post is an overview of the capabilities of Melange and dune together, what's possible to do now and it wasn't with the previous solution (ReScript v9).
...
One blocker for some OCaml developers to use ReScript is the version of the OCaml compiler being stuck in 4.06, a few versions behind latest (5.1) OCaml.

This was an issue in our backend as well, since some packages needed to be compatible for 4.06 and 5.1 and a lot has changed:
...
Dune is the defacto build tool for OCaml development these days. In grosso modo, it’s a wrapper to all OCaml tooling such as Batch compiler (ocamlc), Native compiler (ocamlopt) Library manager (ocamlfind), etc... It abstracts away from those low level tools for users to be in a higher level to define their libraries, executables, testing suites, documentation and other pieces needed to develop a full application.

- Dune allows to rebuild your project smartly, only the libraries that change
- Which means, It would only compile your opam dependencies only once
- Dune wouldn’t build the code where the interface didn’t change
- Dune also builds parallely as much as possible

It also has been expanded to support other platforms than just native OCaml programs such as [js_of_ocaml](https://github.com/ocsigen/js_of_ocaml), [coq](https://coq.inria.fr/), [binding to C](https://michael.bacarella.com/2022/02/19/dune-ctypes/) [unikernels with MirageOS](https://mirage.io/) and now Melange.

[ ] White a small final paragraph as summary/extra
...
- [ ]  Rewrite this *** and mention the downsides of a new tool/environment

The dune integration brings power to users, but with a cost of learning a new tool, **dune**. Being powerful, it’s also a bit  but isn’t perfect as any tool is, and the learning curve might be noticeable.

The initial blocker might be their syntax, which is very lisp-y

- [ ]  Link to the issue/discussion where they explain the syntax wasn’t mean to be public

After working with it almost 4 months I’m convinced that it’s a big improvement over previous status.

It’s mainained by x, y, z where they improve it frequently and they are open for feedback and feature requests (Links to ocaml forum).

To learn more about dune, I recommend → Link to facebook’s intern guy on dune guide
...
ReScript is evolving in a fast pace towards better interoperability with JavaScript and getting ready to compete with TypeScript at feature-level. The ReScript tea is working on v11 which includes an impressive list of features such as:
...
Those features aren’t possible to implement or support with the same design in Melange, since it would mean to break compatibility with OCaml and all the ecosystem.
...
The “beneifts” are very clear based on the experience of using Melange in our ahrefs codebase and other projects. If you are curious on Melange, take a look at the [Getting started guide](https://melange.re/v2.1.0/getting-started)
