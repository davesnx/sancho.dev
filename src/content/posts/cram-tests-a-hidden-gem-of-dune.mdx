...
There are three ways to expose your executable to cram tests:

- First, you can add a `public_name` in your executable stanza like `(public_name my-app)`, which makes the executable available.
- Second, you can set the executable to be part of the dune installation with an [install stanza](https://dune.readthedocs.io/en/latest/reference/dune/install.html) `(install (section bin) (files my_bin.exe))`. Do not confuse dune installation with opam installation, the dune installation offers a way to "copying freshly built artifacts from the workspace to the system", while opam installs opam packages in your opam switch (downloading from opam repositories, building from sources, etc).
- Third, you can directly specify the executable as a dependency in your cram test `(deps %{exe:../bin/my_bin.exe})`. More info in about the `cram` stanza in the [dune documentation](https://dune.readthedocs.io/en/latest/reference/dune/cram.html)
