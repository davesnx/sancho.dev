...
I’m surprised why this pattern is not more common outside of (the small) world of OCaml, since they come from Python ([bitheap.org/cram](https://bitheap.org/cram)) and there are implementations in Go (https://github.com/mgeisler/cram) and . Probably because it didn’t reach to JavaScript world (??).
...
```ocaml
Here there's just a coment about this test, since there's no space at the start
...
// If syntax highlight doesn’t work, screenshot from editor.
...
1. Run the test and let it crash, either
...
To accept the result as correct, we can update the snapshot manually (a bit boring and tedious) or with promotion
(https://dune.readthedocs.io/en/latest/concepts/promotion.html#promotion) (automatic).

1. Running **`dune build @simple —auto-promote`** which updates `simple.t` file for you.
...
- styled-ppx contains a few suites of snapshots (mostly ppx transformations)
    - ppx output in reason https://github.com/davesnx/styled-ppx/tree/main/packages/ppx/test/snapshot/reason
    - ppx output in rescript https://github.com/davesnx/styled-ppx/tree/main/packages/ppx/test/snapshot/rescript
    - css support https://github.com/davesnx/styled-ppx/tree/main/packages/ppx/test/css-support
- reason-react-ppx enforces their ppx generation to be consistent, but also the JavaScript output
    - https://github.com/reasonml/reason-react/tree/main/ppx/test
