...
SKILLS
state: outline
next: post-draft — execute the ASSESSMENT plan: turn the bullets into a release-story (payload/streaming/server-actions deep dive, demo walkthrough, limitations, roadmap); it's a release post about our own RSC implementation, so no research pass
