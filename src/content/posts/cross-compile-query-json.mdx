...
The compilation to JavaScript is the sweet section of this blog post and the part I'm most proud of. It wasn't a big effort, since the tools I used are already mature, but being capable of releasing 2 distributables from a single codebase seems like magic, considering all query-json's dependencies ([menhir](https://opam.ocaml.org/packages/menhir/), [sedlex](https://github.com/ocaml-community/sedlex), and [yojson](https://github.com/ocaml-community/yojson)).
...
After compiling to a JavaScript artifact, I only needed a website where I could run it. I made a playground to teach people how to use it without installing a binary. Aside from the accessibility for users (speed, discoverability, security), it carries a few bonuses, such as testing new versions on each pull request and deploy previews.
