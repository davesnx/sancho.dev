...
This post is about how I rewrote the interpreter using OCaml 5's effect system to handle those control-flow patterns in one model. If you've ever implemented `try/catch` in a language, you already know the shape of the problem. If not, don't worry — we'll walk through it step by step.

## Before and after, quickly

Before the refactor, control flow was spread across return values and special-case branching. That worked for a subset of jq, but got brittle as language features expanded.

After the refactor, control flow is represented by four effects:

- `Yield` for producing values (zero, one, many).
- `Break` for loop exit.
- `Halt` for terminating program execution with an exit code.
- `User_error` for user-visible errors caught by `try/catch`.

The main practical change is that interpreter cases stay focused on local semantics, while handlers manage cross-cutting flow concerns.
...
These are all **effects**, and the handlers are where non-local control flow is defined.
...
These four effects model the interpreter's control flow:

- `Yield` -> produce results
- `Break` -> stop loops
- `Halt` -> terminate program execution
- `User_error` -> raise user-program errors

### Visual mental model

If you plan to add a diagram later, this is the shape:

1. A central `interp` loop performs one of four effects.
2. Nearby handlers catch only what they care about (`Yield` in `pipe`, `User_error` in `try/catch`, etc.).
3. Unhandled effects bubble to outer handlers.
4. The top-level `execute` handler is the final boundary.
...
## Why effects over exceptions or result plumbing
...
For this interpreter, effects reduced the amount of cross-function plumbing in control-flow-heavy paths while keeping the top-level model explicit.

## Performance notes from benchmarks

Performance was one of the first questions I had after this refactor, so I leaned on the benchmark suite in the query-json repo (`benchmarks/README.md`). On the published benchmark set (query-json `0.5.52` vs jq `1.8.1`), query-json is generally faster, with reported speedups between roughly `1.5x` and `4.5x` depending on operation and file size.

A few examples from that report:

- Small file (`1.3KB`), `.first.id`: around `3.0x` faster.
- Medium file (`104KB`), `length`: around `4.5x` faster.
- Big file (`575KB`), `.[0]`: around `3.3x` faster.
- Huge file (`97MB`), identity (`.`): around `1.5x` faster.

This does **not** prove effects are the reason for all wins. The benchmark report also points to native compilation, parser/runtime choices, and architecture differences. The practical takeaway for this article is narrower: moving control-flow glue to effects did not block strong performance in real workloads.

## Where effects actually matter

It's worth being explicit about where effects pull their weight. Most of the interpreter — `map`, `sort_by`, `group_by`, `keys`, math operations — eagerly collects results into lists via `collect` and then works on those lists directly. Those functions could return `Json.t list` and be fine.
