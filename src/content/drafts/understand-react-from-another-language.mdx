...
SKILLS
state: full-draft
next: post-sand — talk script written start to finish (4 acts plus slide/demo/Q&A appendices); prose needs tightening, not drafting
then: post-review
*/}
...
React taught a generation of JavaScript developers to think functionally. Immutability. Pure functions. Composition. But here's the thing—React was always holding back. It was born in JavaScript, so it had to compromise.

Tonight, I want to show you what React looks like without those compromises.

I used React for years. I built apps, learned hooks, understood the mental model—or so I thought. Then I fell down a rabbit hole. I started learning functional programming languages. First Elm, then Reason, then OCaml. Somewhere along the way, I stopped just using React. I started reimplementing it.
...
React's concepts came from ML family languages—Standard ML, OCaml. The first React prototype was actually written in Standard ML. Components, immutability, pure render functions—these aren't JavaScript ideas. They're functional programming ideas that Jordan smuggled into JavaScript.
...
Let me introduce you to OCaml. Or rather, Reason—a syntax for OCaml that looks familiar to JavaScript developers.
...
**Types are inferred.** You don't write annotations everywhere—the compiler figures it out:
...
- `~name` is a labeled argument. You literally cannot forget to pass it—the compiler stops you.
...
- `Empty` → That's `null` in React
- `Text(string)` → Text nodes, like `{React.string("hello")}`
- `Fragment(element)` → `<React.Fragment>` or `<>`
- `Html_element(string, attributes, list(element))` → Every `<div>`, `<span>`, `<button>`
- `Component(unit => element)` → Your components—they're just functions that return elements
- `Provider(element)` and `Consumer(element)` → Context
- `Suspense({ children: element, fallback: element })` → Look how simple this is!
...
Components can actually pause and resume. No hacks. No throwing promises. That's not sci-fi—that's what React is trying to approximate with generators and async iteration.
...
└─────────────────────────────────────────────────────────────┘
              ...
              ▼                               ▼
┌─────────────────────────┐     ┌─────────────────────────┐
│       Melange           │     │   server-reason-react   │
│   (compiles to JS)      │     │   (native OCaml)        │
└─────────────────────────┘     └─────────────────────────┘
              │                               │
              ▼                               ▼
┌─────────────────────────┐     ┌─────────────────────────┐
│       Browser           │     │     Native Server       │
│   (hydrates, interactive)│     │   (renders HTML)        │
└─────────────────────────┘     └─────────────────────────┘
```
...
The `$L1` is a reference—"this part isn't ready yet, I'll send it later." That's streaming. That's Suspense on the wire.
...
When you wrap something in Suspense, you're saying "show this OR that." There's no magic scheduler making decisions—there's a condition being checked. Understanding this simplifies your mental model.
...
The element type shows that React is simpler than it appears. Components, elements, fragments—they're all the same recursive structure. A component is just a function. An element is just data. A tree is just nested data.
...
React is converging toward ideas that OCaml has had for 30 years. The React team knows this—they've talked about it in conferences. We're not predicting the future; we're implementing it in a language that already has the primitives.
...
1996 ─── OCaml created (INRIA, France)
         │
2011 ─── Jordan Walke experiments with SML + React ideas
         │
2013 ─── React announced at JSConf
         │
2016 ─── Reason syntax created (by Jordan Walke)
         │
2023 ─── server-reason-react ships at Ahrefs
         │
2026 ─── You're here
```
...
│           Reason                │          JavaScript             │
├─────────────────────────────────┼─────────────────────────────────┤
│ let greeting = name =>          │ const greeting = (name) =>      │
│   "Hello, " ++ name;            │   "Hello, " + name;             │
├─────────────────────────────────┼─────────────────────────────────┤
│ let add = (a, b) => a + b;      │ const add = (a, b) => a + b;    │
│ // Compiler knows: int → int    │ // Who knows? 🤷                │
└─────────────────────────────────┴─────────────────────────────────┘
```
...
**Speaker note:** "JSX works. Props work. But notice the `~` — that's a labeled argument. You can't forget it."
...
type element =
  ...
  | Text(string)                             ← "hello"
  | Fragment(element)                        ← <>...</>
  | Html_element(string, attrs, children)    ← <div>
  | Component(unit => element)               ← <YourComponent />
  | Provider(element)                        ← Context.Provider
  | Consumer(element)                        ← Context.Consumer
  | Suspense({ children, fallback })         ← <Suspense>
```
...
```
┌────────────────────────┬──────────────┬─────────────┐
│ Stack                  │ Requests/sec │ vs Node     │
├────────────────────────┼──────────────┼─────────────┤
│ Node.js (Express)      │     7,200    │    1x       │
│ Bun                    │    10,300    │   1.4x      │
│ server-reason-react    │    64,800    │   9x        │
└────────────────────────┴──────────────┴─────────────┘
```
...
```
0:["$","div",null,{"children":[        ← Initial shell
    ["$","h1",null,{"children":"Hello"}],
    ["$","$Suspense",null,{
      "fallback":"Loading...",
      "children":"$L1"                  ← "I'll send this later"
    }]
  ]}]

1:["$","UserProfile",null,{            ← Streamed when ready
    "data":{"name":"David"}
  }]
```
...
```
1. Think in types first
   → Sketch the type signature before coding

2. Suspense is just branching
   → "Show this OR that" — no magic

3. Understand your abstractions
   → Components are functions, elements are data

4. Streaming is sequential
   → Render, send, render more, send more
```
...
TypeScript bolts types onto JavaScript after the fact. The types are optional and can be bypassed. OCaml's types are structural—they're not an afterthought, they're how the language thinks. But more importantly: this talk isn't about convincing you to switch languages. It's about what you learn when you see React's ideas expressed in their natural habitat.
...
No. The point is the mental model. If this talk makes you curious about what `useState` really does, or why Suspense works the way it does, it succeeded. Whether you explore that in OCaml, by reading React's source, or by building your own mini-React in JavaScript—that's up to you.
