...
import { RepoGallery } from '@/components/repo-gallery';
...
const getOpenSourceRepos = (items: GitHubRepo[]): GitHubRepo[] => {
  ...
  const merged = [
    ...
    ...items.filter((repo) => !featuredNames.has(repo.fullName)),
  ];
...
const CompanyLogo = ({ logo }: { logo: string | null }) => {
  if (!logo) {
    return <p style={{ fontSize: fonts.fontSize3, lineHeight: 1, margin: 0, padding: 0 }}>😭</p>;
  }
...
export const metadata: Metadata = buildMetadata({
  title: 'About',
  description:
    'About David Sancho: software engineer working on OCaml, developer tooling, and UI infrastructure at ahrefs. Work history and open source projects.',
  path: '/about',
});
    ...
    <Page title={<H1>About</H1>}>
      <div className={contentClass}>
        <Text size={fonts.fontSize1} align="left">
          ...
          <IconTextLink
            ...
            icon="https://avatars.githubusercontent.com/u/20414525?s=80&v=4"
            aria-label="Reason"
          >
            Reason
          </IconTextLink>
      ...
      <div className={contentClass}>
        <Text size={fonts.fontSize1}>
          ...
          {
            ', building the UI infrastructure, always at the intersection between backend, frontend and compilers. Previously, I helped build visual app development platforms at '
          }
          <IconTextLink href="https://draftbit.com" icon="/logos/draftbit-icon.svg" aria-label="Draftbit">
            Draftbit
          </IconTextLink>
          {' for a year and, before that, worked at '}
          <IconTextLink href="https://www.typeform.com" icon="/logos/typeform-icon.svg" aria-label="Typeform">
            Typeform
          </IconTextLink>
      ...
      <Stack gap={3} align="left">
        {jobs.map((job) => (
          <Job
            ...
            company={{ name: job.company, url: job.url, logo: job.logo }}
            date={{ from: job.from, to: job.to }}
            role={job.role}
          />
        ))}
      </Stack>
      ...
      <div className={contentClass}>
        <Text size={fonts.fontSize1}>
          ...
          {', where we talk with the authors and maintainers behind ML-family languages and their tooling.'}
        </Text>
      </div>
      ...
      <RepoGallery repos={openSourceRepos} />
