...
const OpenSource = ({ repo }: { repo: GitHubRepo }) => (
  <a className={openSourceItemClass} href={repo.url} target="_blank" rel="noopener noreferrer">
    <div className={repoHeaderClass}>
      {/* biome-ignore lint/performance/noImgElement: Remote GitHub avatars are small generated metadata with no stable dimensions source. */}
    ...
    <div className={repoFooterClass}>
      {repo.language ? (
        <div className={statItemClass}>
          ...
          <Text className="repo-meta" monospace weight={600} size={font.fontSize0} color={colors.textMuted}>
            {repo.language}
          </Text>
        </div>
      ) : null}
...
export const metadata = buildMetadata({
  title: 'Work',
  description:
    'Work history and open source projects by David Sancho, from ahrefs and Typeform to OCaml tooling and libraries.',
  path: '/work',
});
