...
export const buildPostLlmText = (post: BlogPost): string => {
  ...
  if (post.kind === 'youtube') {
    ...
    lines.push(`- **Event**: ${post.event}`);
    lines.push(`- **Uploaded**: ${post.uploadedAt}`);
    lines.push(`- **Video**: https://www.youtube.com/watch?v=${post.youtubeId}`);
  }
  ...
  lines.push('---');
  lines.push('');
  const content = stripMdxComponents(post.content);
  if (post.kind === 'youtube') {
    lines.push(post.description);
    if (content) lines.push('', '## Transcript', '', content);
  } else {
    lines.push(content);
  }
