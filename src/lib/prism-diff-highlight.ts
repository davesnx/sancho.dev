...
export default function diffHighlight(Prism) {
  ...
  const HTML_LINE = RegExp(
    /(?:__|[^\r\n<])*(?:\r\n?|\n|(?:__|[^\r\n<])(?![^\r\n]))/.source.replace(/__/g, function match() {
      return HTML_TAG.source;
    }),
    'gi',
  );

  const { PREFIXES } = Prism.languages.diff;
  ...
  Prism.hooks.add('wrap', (env) => {
    ...
    if (env.type in PREFIXES) {
      ...
      const highlighted = diffGrammar ? Prism.highlight(code, diffGrammar, diffLanguage) : Prism.util.encode(code);
      ...
      let match = HTML_LINE.exec(highlighted);
      while (match) {
        lines.push(prefix + match[0]);
        match = HTML_LINE.exec(highlighted);
      }
      if (/(?:^|[\r\n]).$/.test(decoded)) {
