...
import { getAboutContent, getPublishedBlogPosts, type PublishedBlogPost } from './get-pages.mts';
...
export const generateLLMsTxt = async (): Promise<string> => {
  ...
  const sortedPosts = posts.sort((a, b) => compareDesc(parseISO(a.publishedAt), parseISO(b.publishedAt)));
  ...
  for (const post of sortedPosts) {
    ...
    lines.push(`- [${post.title}](https://sancho.dev${post.route}.md) - ${post.publishedAt}${tagSuffix}`);
  }

  return `${lines.join('\n')}\n`;
};

export const generateMarkdownFiles = async (outputDir = Path.join(process.cwd(), 'out')): Promise<void> => {
  const posts = await getPublishedBlogPosts();
