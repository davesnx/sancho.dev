...
const isExcludedPage = (page: string): boolean => page.includes('pages/404.') || page.includes('pages/blog/[...slug].');

const getRequiredField = (post: BlogPostPage, field: 'title' | 'publishedAt'): string => {
...
export const getBlogPages = async (): Promise<BlogPostPage[]> => {
  const pages = await globby(['src/content/posts/**/*.mdx', 'src/content/posts/**/*.md']);
  ...
  return pages
    ...
    .map((page) => {
      ...
      const slug = page.replace('src/content/posts/', '').replace(/\.(mdx|md)$/, '');
...
export const getPublishedBlogPosts = async (): Promise<PublishedBlogPost[]> => {
  ...
  return posts
    .filter((post) => post.draft !== true && post.published !== false && !post.canonicalUrl)
    .map((post) => ({
      ...post,
